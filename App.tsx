...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('elizapad_launches');
    if (saved) {
      try {
      ...
      setLaunches(DEFAULT_LAUNCHES);
      localStorage.setItem('elizapad_launches', JSON.stringify(DEFAULT_LAUNCHES));
    }
  }, []);
  ...
  const connectWallet = async (type: WalletType) => {
    ...
    try {
      ...
      // Restore simulated connection as a fallback if extension is not present
      // This ensures the demo app remains functional for users without extensions
      ...
      // If user cancelled, just close the modal
      if (err?.code === 4001) {
        setIsWalletModalOpen(false);
      } else {
        // Force fallback if extension exists but failed for some reason (other than rejection)
        const mockAddress = '8fGx' + Math.random().toString(36).substring(2, 8).toUpperCase() + 'kL9';
  ...
  const handleNewLaunch = (coin: Coin) => {
    ...
    setLaunches(updated);
    localStorage.setItem('elizapad_launches', JSON.stringify(updated));
  };
          ...
          <Routes>
            ...
            <Route path="/launches" element={
              <div className="container mx-auto px-4 py-12">
                <LatestLaunches launches={launches} title="All Eliza-Verified Launches" />
              </div>
            } />
        ...
        <footer className="relative z-10 py-12 border-t border-white/5 bg-black/20 backdrop-blur-md mt-20">
          <div className="container mx-auto px-4 text-center">
            <p className="text-[#9BA1A6] text-sm font-orbitron">
              ELIZAPAD © 2024 • AI-CURATED SOLANA ECOSYSTEM
            </p>
